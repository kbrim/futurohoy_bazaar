...
const getTopNewProducts = cache(async () => {
  const response = await axios.get<Product[]>(
    "/api/furniture-1/products?tag=new",
  );
...
const getTopSellingProducts = cache(async () => {
  const response = await axios.get<Product[]>(
    "/api/furniture-1/products?tag=top-selling",
  );
  return response.data;
});
...
const getFurnitureShopNavList = cache(async () => {
  const response = await axios.get<CategoryNavList[]>(
    "/api/furniture-1/navigation",
  );
  return response.data;
});

const getMainCarouselData = cache(async () => {
  const response = await axios.get<FurnitureCarouselItem[]>(
    "/api/furniture-1/main-carousel",
  );
  return response.data;
});

const getCategory = cache(
  async (category: string): Promise<{ title: string }> => {
